...
@interface ABRetrievePasswordView()<UITextFieldDelegate>
...
@property(nonatomic,strong) UIButton *resetPasswordButton;   // 重置密码按钮
...
@implementation ABRetrievePasswordView
...
//    [self addSubview:self.descLabel];
    ...
    // 2.3 密码TextField
    self.passworkTextField.width = ScreenWidth - PADDING_30PX * 4;
    ...
    // 2.4 注册按钮
    self.resetPasswordButton.width = ScreenWidth - 2 * PADDING_RIGISTERBUTTON_LEFT;
    self.resetPasswordButton.height = SIZE_HEIGHT_TEXTFILED;
    self.resetPasswordButton.y = self.passworkTextField.bottom + PADDING_REGISTERBUTTON_TOP;
    self.resetPasswordButton.centerX = ScreenWidth / 2;
...
- (void)resetPasswordButtonClick {
    ...
    NSString *urlStr = @"customer/v1.0.1/forgetPassword";
    ...
    NSDictionary *dict = [NSDictionary dictionaryWithObjects:@[phone,password,code] forKeys:@[@"phone",@"password",@"code"]];
    ...
    [KABNetworkManager POST:urlStr parameters:dict success:^(id responseObject) {
        ...
        if([@"200" isEqualToString:responseObject[@"errcode"]]){
            [CustomToast showDialog:@"重置密码成功" time:1.5];
            // 跳转到用户登录界面
        ...
        if (isDisConnectedNetwork) {
            ...
            [CustomToast showDialog:@"重置密码失败" time:1.5];
        }
        return ;
    }];
...
- (void)getCodeButtonDidClick {
    ...
    if ([@"" isEqualToString:self.accountTextField.text]) {
        ...
        NSDictionary *dict = [NSDictionary dictionaryWithObjects:@[phoneNum,@"1"] forKeys:@[@"phone",@"regstatus"]];
        // 调用接口获取验证码 - 此接口会判断手机号是否注册 ，common/captcha/v1.0.1/sendSms不会判断
...
#pragma mark - ‘重置密码’按钮状态监听
- (void)changeResetButtonState {
    ...
    if (hasMobile && hasPsssword && hasIdentifyingCode) {
        ...
        self.resetPasswordButton.backgroundColor = KColorBlue;
    } else {
        self.resetPasswordButton.enabled = NO;
        self.resetPasswordButton.backgroundColor = KDisableColor;
    }
}
...
- (HFHTextField *)accountTextField {
    if (!_accountTextField) {
        ...
        [_accountTextField addTarget:self action:@selector(changeResetButtonState) forControlEvents:UIControlEventEditingChanged];
...
- (HFHTextField *)passworkTextField {
    if (!_passworkTextField) {
        ...
        [_passworkTextField addTarget:self action:@selector(changeResetButtonState) forControlEvents:UIControlEventEditingChanged];
    }
...
- (UIButton *)resetPasswordButton {
    if (!_resetPasswordButton) {
        _resetPasswordButton = [UIButton buttonWithTitle:@"重置密码" color:KWhiteColor fontSize:FONT_SIZE_BASCICBUTTON imageName:nil];
        _resetPasswordButton.backgroundColor = KDisableColor;
        [_resetPasswordButton.layer setCornerRadius:5];
